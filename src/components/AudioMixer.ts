// AudioMixer.ts - Audio mixing logic using Web Audio API
...
export class AudioMixer {
  ...
  async connectMicrophoneStream(deviceId: string): Promise<boolean> {
    try {
  ...
  async connectSystemStream(deviceId: string): Promise<boolean> {
    try {
      if (!this.audioContext) {
        throw new Error('AudioContext not initialized')
      }
  ...
  async setupDeepgramStreaming(deepgramClient: DeepgramClient): Promise<MediaRecorder> {
    if (!this.mixedStream) {
