...
function AppContent() {
  ...
  useEffect(() => {
    if (settings && !settingsLoading) {
      setAppState(prev => ({
        ...
        outputFolder: settings.defaults.outputFolder || prev.outputFolder || null,
      ...
      try {
        if (window.electronAPI && !appState.outputFolder) {
          ...
          if (result.success && result.path) {
            setAppState(prev => ({ ...prev, outputFolder: result.path || null }))
          }
        }
      } catch (error) {
  ...
  // Recording hook
  const { startRecording: startRecordingHook, stopRecording: stopRecordingHook } = useRecording({
    onTranslationReceived: useCallback((line: TranslationLine) => {
    ...
    onStatusChange: useCallback((status: 'READY' | 'CONNECTING' | 'LISTENING' | 'RECONNECTING' | 'ERROR') => {
  ...
  }, [stopRecordingHook, appState.outputFolder, appState.sessionName, appState.direction])
